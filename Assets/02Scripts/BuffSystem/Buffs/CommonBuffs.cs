...
public class MoveSpeedBuff : TimerBuff
{
...
/*
    ...
    일단 버프를 부여하는 예시를 보여드리자면,
        플레이어 (혹은 적)의 이동속력을 10초 동안 20% 감소시키는 버프를 부여하고 싶다면...
          1) 미리 BuffSystem 클래스에서 _BuffDictionary 컨테이너에 <버프 코드, new MoveSpeedBuff(null, 0.2f, 10.0f)> 를 추가하고 (BuffSystem 클래스의 LoadAllBuffs 메서드 참고),
          2) PlayerCharacter (적에게 부여하는 경우, Enemy) 컴포넌트에 BuffSystem 객체 생성.
          3) PlayerCharacter.AddBuff(버프 코드) (적에게 부여하는 경우, Enemy.AddBuff(버프 코드)) 호출
         위 세 단계에 걸쳐 이동 속도 디버프를 부여할 수 있습니다.
    
    다만, 버프의 효과를 나타내는 코드를 작성하는 부분은 Buff 클래스 내의 OnStartBuff, OnRenewBuff, OnUpdateBuff, OnFinishedBuff 메서드들입니다.
    즉, 이동속도 감소 코드를 MoveSpeedBuff 내부에서 작성해야합니다.
    ...
    예) IVelocity 라는 인터페이스를 정의하여, 채이님이 정의한 클래스에 적용시키는 예시
